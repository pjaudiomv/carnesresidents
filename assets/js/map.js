...
var googleMap = {
	...
	initialize: function () {
		...
		// set defaults
		var center = (force_center) ?  $("[data-map][data-center]").data('center') : [33.053664,-80.106505];
	...
	loadMap: function (json_file_path) {
		...
		script.src = 'https://maps.googleapis.com/maps/api/js?key=AIzaSyDolzT1yWJuPi6y3tP85fuh3JbPJfJnWeM&callback=googleMap.initialize';
	...
	buildMarkersFromJSON: function (callback) {
		$.get(json_file_path, function (data) {
			...
			for(var i=0; i < markers.length; i++) {
				self.markers[i] = self.createMarker(new google.maps.LatLng(markers[i]['lat'],markers[i]['lng']), markers[i], markers[i]['group']);
			}
			callback();
		});
	},
	...
	buildMarkersFromMarkup: function (callback) {
		$('[data-lat]').each(function(i,e) {
			...
			self.markers[i] = self.createMarker(new google.maps.LatLng(lat,lng), title);
		});
		callback();
	},
	/**
	...
	toggleFilters: function () {
		var groups = $("[data-map-filters] input[type=checkbox]:checked").map(function() { return $(this).val() }).get();
...
$(document).ready(function () {
	if ($('[data-map]').length) { googleMap.loadMap('assets/data/markers.json') };
